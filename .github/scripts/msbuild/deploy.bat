...
PsExec64.exe --nobanner -accepteula \\%SERVER% -u %DESTINATION_USER% -p "%DESTINATION_PWD%" "C:\Windows\System32\inetsrv\appcmd stop apppool /apppool.name:%APP_NAME%"
X:
...
PsExec64.exe --nobanner -accepteula \\%SERVER% -u %DESTINATION_USER% -p "%DESTINATION_PWD%" "C:\Windows\System32\inetsrv\appcmd start apppool /apppool.name:%APP_NAME%"
