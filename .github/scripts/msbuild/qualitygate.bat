@echo off
set SONAR_TOKEN="c3FhX2Y1MDVhYTA4YTRhNmJmNzI0MDI0YThmZDBkNTBhN2RiN2RkYWQyNzA6"
...
echo %url%
curl -v --header "Authorization: Basic %SONAR_TOKEN%" --location %url% 
curl -s --header "Authorization: Basic %SONAR_TOKEN%" --location %url% > output.json
